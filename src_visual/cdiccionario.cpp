...
bool CDiccionario::nuevo_Diccionario(string nombre)
{
    bool creado = false;
    ...
    this->ptr_Archivo = fopen(nombre.c_str(), "w+b");
    /*Si el archivo se pudo crear*/
    if(this->ptr_Archivo != NULL)
    {
        qDebug() << "[DEBUG] "<< "Diccionario" << " Creado!!";
    ...
    {
        qDebug() << "[DEBUG] "<< "No se pudo crear el Diccionario";
    }
    return creado;
}

void CDiccionario::ponTabla_Atributos(QTableWidget *tabla, int indexEntidad)
{
    list<CAtributo> *lista_aux;
    list<CAtributo>::iterator iAtributo;
    list<CEntidad>::iterator iEntidad;
    QTableWidgetItem *item;
    QString aux_item;

    int con = 0;

    iEntidad = this->lista_entidades.begin();
    if(iEntidad != this->lista_entidades.end())
    {
        advance(iEntidad, indexEntidad);
        lista_aux = iEntidad->Dame_listaAtr();
        iAtributo = lista_aux->begin();

        while(iAtributo != lista_aux->end())
        {
            tabla->insertRow(con);
            for(int i = 0; i < 5; i++)
            {
                switch (i) {
                case 0:
                    aux_item = iAtributo->dame_Nombre();
                    item = new QTableWidgetItem(aux_item);
                    break;
                case 1:
                    aux_item = QString::number(iAtributo->dame_Tipo());
                    item = new QTableWidgetItem(aux_item);
                    break;
                case 2:
                    aux_item = QString::number(iAtributo->dame_Tamanio());
                    item = new QTableWidgetItem(aux_item);
                    break;
                case 3:
                    aux_item = QString(iAtributo->dame_Indice());
                    item = new QTableWidgetItem(aux_item);
                    if(iAtributo->dame_Indice())
                    {
                    ...
                    break;
                case 4:
                    aux_item = QString(iAtributo->dame_ClavePrimaria());
                    item = new QTableWidgetItem(aux_item);
                    if(iAtributo->dame_ClavePrimaria())
                    {
                        item->setCheckState(Qt::Checked);
                    }
                    else
...
QStringList CDiccionario::Dame_Entidades()
{
    QStringList entidades;
    ...
    {
        entidades.append(iterador->dame_Nombre());
        iterador++;
    }
    if(this->cabecera == -1)
    {
...
bool CDiccionario::abrir_Diccionario(string nombre)
{
    bool abierto = false;
    ...
    this->ptr_Archivo = fopen(nombre.c_str(), "r+b");
    /*Si el archivo se pudo abrir se cargan sus datos*/
    if(this->ptr_Archivo != NULL)
    {
        qDebug() << "[DEBUG] " << "Diccionario " << "Abierto!!";
        ...
        qDebug() << "[DEBUG] " << "Cabecera en " << this->cabecera;
...
/*string CDiccionario::edita_Entidad(int index, char n[20])
{
    ...
    {
        iterador = this->lista_entidades.begin();
        //Se verifica si la lista no esta vacia comparado el interador con el final de la lista
        if(iterador != this->lista_entidades.end())
        {
            El metodo advance el iterador n veces si seleccione el elemento 2 advance recorre el iterador 2 veces
...
{
    Menu principal donde esta seleecionado una entidad para agregarle sus atributos
